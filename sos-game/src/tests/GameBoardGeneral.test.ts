...
test("If I make an SOS, I should get another turn", () => {
    let board = new GameBoardGeneral(3);
    board.setPlayerMarker(Player.Blue, TileContent.S);
    board.setPlayerMarker(Player.Red, TileContent.O);
    board.makeNextMove(0, 0); //Blue
    board.makeNextMove(1, 1); //Red
    board.makeNextMove(2, 2); //Blue, but should get another turn for completing SOS

    expect(board.getPlayerWithNextMove()).toBe(Player.Blue);
});

test("If blue makes more SOSes than red, blue should win", () => {
    let board = new GameBoardGeneral(3);
    board.setPlayerMarker(Player.Blue, TileContent.S);
    board.setPlayerMarker(Player.Red, TileContent.O);
    board.makeNextMove(0, 0); //Blue
    board.makeNextMove(0, 1); //Red
    board.makeNextMove(0, 2); //Blue, but should get another turn for completing SOS
    
    board.makeNextMove(1, 0); //Blue
    board.makeNextMove(1, 1); //Red
    board.makeNextMove(1, 2); //Blue

    board.makeNextMove(2, 0); //Red
    board.makeNextMove(2, 1); //Blue
    board.makeNextMove(2, 2); //Red

    expect(board.getGameWinner()).toBe(Player.Blue);

});

test("If red makes more SOSes than blue, red should win", () => {
    let board = new GameBoardGeneral(3);
...
test("If neither the number of SOSes created between red and blue is equal, the game is a draw", () => {
    let board = new GameBoardGeneral(3);
    board.makeNextMove(0, 0);
    board.makeNextMove(1, 1);
    board.makeNextMove(2, 2);
